...
public class ResAndStatsTest5_16 extends ResAndStats implements ResAndStatsTestCase {
    ...
    public void saveTestCase(String[] roundResult, List<ResAndStatsTestCase> resAndStatsTestCaseList) {
        if (Helper.find(Integer.parseInt(roundResult[0]), oldColdNumberList) && !isInitialize) {
            ...
            resAndStatsTestCaseList.removeIf(resAndStats -> resAndStats instanceof ResAndStatsTest5_16 && resAndStats.getTestCase() == testCase
    ...
    private static boolean isPassed(TestResult result) {
        ...
        if (Helper.find(numberResult, actualNumberResultList)) return false;

...
public class PayoutTest17 extends Payout implements PayoutCase {
    ...
    private final int testCase = 17, payoutOdds = 8;
    private final int[][][] betOptionList = Arrays.copyOf(BettingOption.CORNER_LIST, BettingOption.CORNER_LIST.length);
    ...
    public void setBetOption() {
        ...
        for (int[][] betOption : betOptionList)
            EventHandler.click(DealerTable.BettingOption.getCorner(betOption));
    }
    ...
    public void getBetOption() {
        ...
        byte i = 0;
        for (int[][] betOption : betOptionList)
            chipValueList[i++] = getChipValue(DealerTable.BettingChip.getCorner(betOption));
    }

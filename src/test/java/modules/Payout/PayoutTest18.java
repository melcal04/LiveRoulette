...
public class PayoutTest18 extends Payout implements PayoutTestCase {

    private double bet, payout;
    private final int testCase = 18, payoutOdds = 8;
    private final int[][][] betOptionList = Arrays.copyOf(TestVariables.ZERO_SECTION_LIST, TestVariables.ZERO_SECTION_LIST.length);
    ...
    public void placeBettingOption() {
        ...
        for (int[][] betOption : betOptionList)
            EventHandler.click(BettingPanel.Option.getZeroSection(betOption));
    }
    ...
    public void checkBettingChip() {
        if (isZeroSectionPlaced) {
            int i = 0;
            for (int[][] betOption : betOptionList)
                chipValueList[i++] = getChipValue(BettingPanel.Chip.getZeroSection(betOption));
        }
    }
        ...
        if (isZeroSectionPlaced) {
            ...
            for (int[][] betOption : betOptionList) {
                if (TestConditions.isPosBetWin(roundResult, betOption)) {
                    ...
                    addWin("Zero Section #" + betOption[1][0] + ", #" + betOption[1][1]  +
    ...
    public void saveTestCase(String[] roundResult, List<PayoutTestCase> payoutTestCaseList) {
        if (isZeroSectionPlaced) {
            ...
            payoutTestCaseList.removeIf(payout -> payout instanceof PayoutTest18 && payout.getTestCase() == testCase);
        }
    }

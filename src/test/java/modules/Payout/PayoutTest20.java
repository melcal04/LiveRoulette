...
public class PayoutTest20 extends Payout implements PayoutCase {

    private double bet, payout;
    private final int testCase = 20, payoutOdds = 11;
    private final int[][][] betOptionList = Arrays.copyOf(BettingOption.ZERO_CORNER_LIST, BettingOption.ZERO_CORNER_LIST.length);
    ...
    public void setBetOption() {
        ...
        for (int[][] betOption : betOptionList)
            EventHandler.click(DealerTable.BettingOption.getZeroCorner(betOption));
    }
    ...
    public void getBetOption() {
        ...
        byte i = 0;
        for (int[][] betOption : betOptionList)
            chipValueList[i++] = getChipValue(DealerTable.BettingChip.getZeroCorner(betOption));
    }
